...
from tuxemon.sprite import GridLayout


def roundtrip_lr_tb(count, columns, rectangular, orientation):
...
def roundtrip_tb_lr(count, columns, rectangular, orientation):
    layout = GridLayout(count, columns, rectangular)
    # TB index may exceed count in rectangular mode
    max_tb = layout.rows * columns if rectangular else count
...
def test_lr_tb_roundtrip(count, columns, rectangular, orientation):
    roundtrip_lr_tb(count, columns, rectangular, orientation)
...
@pytest.mark.parametrize("rectangular", [False, True])
@pytest.mark.parametrize("orientation", ["horizontal"])
def test_tb_lr_roundtrip(count, columns, rectangular, orientation):
    roundtrip_tb_lr(count, columns, rectangular, orientation)
